...
var CustomAuthProvider = () => {
  if (typeof window === "undefined") {
    return {
      ...
      fetchWithToken: async (input, init) => fetch(input, init),
      getLoginStrategy: () => "Redirect",
      getLoginScreen: () => null,
      getSessionProvider: () => (props) => props.children
    };
  }
  return {
    authenticate: async () => {
      ...
      try {
        ...
        if (response.ok) {
          const { token: token2 } = await response.json();
          localStorage.setItem("tina_jwt", token2);
